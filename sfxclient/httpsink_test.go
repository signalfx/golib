...
func TestHTTPDatapointSink(t *testing.T) {
	Convey("dim cache panics on double init", t, func() {
		s := NewHTTPSink()
		s.InitDimensionCache()
	...
	for _, caching := range []bool{true, false} {
		var suffix string
		if caching {
			suffix = " with dim cache"
		} else {
			suffix = "without dim cache"
		}
		Convey("A default sink"+suffix, t, func() {
			s := NewHTTPSink()
			if caching {
				s.InitDimensionCache()
			}
			...
			Convey("mapToDimensions should filter empty", func() {
				So(len(s.mapToDimensions(map[string]string{"": "hi"})), ShouldEqual, 0)
			})
			Convey("with a test endpoint", func() {
				...
				Convey("sink without cache should send dimensions", func() {
				})
				Reset(func() {
					if blockResponse != nil {
						close(blockResponse)
