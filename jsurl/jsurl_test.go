...
func TestParse(t *testing.T) {
	tests := []struct {
		...
		// Invalid syntax
		{nil, "(a~1)", true},
		// "bad" is not a valid literal
		{nil, "~(a~bad)", true},
		// Bad unicode value
		{nil, "~'hello*2gworld**203c", true},
		// Bad unicode value
		{nil, "~'hello*20world**2l3c", true},
		{nil, "~(~l)", true},
		{nil, "~(*5la~hello~b~%2525252527world)", true},
		{nil, "~(~", true},
		{nil, "~(a~%27hello~b~%27world", true},
	}
	...
	for _, tt := range tests {
		...
		if tt.expectError {
			require.NotNil(t, err, "got %v as return value", parsed)
		} else {
			require.Nil(t, err)
		}
